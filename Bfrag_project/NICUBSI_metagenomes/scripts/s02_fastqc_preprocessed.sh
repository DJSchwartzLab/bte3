...
sample=`sed -n ${SLURM_ARRAY_TASK_ID}p ${BASEDIR}/NICUBSI_metagenomes_mappingfile.txt`

# Run FastQC on both forward and reverse reads
set -x
time fastqc ${INDIR}/${sample}_FW_CLEAN.fastq ${INDIR}/${sample}_RV_CLEAN.fastq -o ${OUTDIR}
RC=$?
set +x
