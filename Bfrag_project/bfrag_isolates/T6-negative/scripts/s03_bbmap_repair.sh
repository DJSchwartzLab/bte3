...
BASEDIR="/scratch/djslab/jltung/Bfrag_project/bfrag_isolates/T6-negative"
...
ID=`sed -n ${SLURM_ARRAY_TASK_ID}p ${BASEDIR}/T6-negative_mappingfile.txt`

#Run repair
repair.sh --tossbrokenreads \
    in1=${INDIR}/${ID}_FW.fastq \
    in2=${INDIR}/${ID}_RV.fastq \
    out1=${OUTDIR}/${ID}_FW_REPAIRED.fastq \
    out2=${OUTDIR}/${ID}_RV_REPAIRED.fastq \
    outs=${OUTDIR2}/${ID}_SINGLETONS_REPAIRED.fastq.gz