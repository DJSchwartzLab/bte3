...
#==================================================================================
# File name    : s05_fastqc_preprocessed.sh
# Description  : This script will run FastQC on samples in parallel.
# Usage        : sbatch s05_fastqc_preprocessed.sh
...
sample=`sed -n ${SLURM_ARRAY_TASK_ID}p ${BASEDIR}/HMP_metagenomes_mappingfile.txt`

# Run FastQC on both forward and reverse reads
set -x
time fastqc ${INDIR}/${sample}.denovo_duplicates_marked.trimmed.1.fastq ${INDIR}/${sample}.denovo_duplicates_marked.trimmed.2.fastq -o ${OUTDIR}
RC=$?
set +x
