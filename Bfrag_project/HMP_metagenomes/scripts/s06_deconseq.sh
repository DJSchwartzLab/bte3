...
INDIR="/scratch/djslab/jltung/Bfrag_project/HMP_metagenomes/downloaded_reads"
OUTDIR="/scratch/djslab/jltung/Bfrag_project/HMP_metagenomes/deconseq_out"

# Create output directory if it doesn't exist
mkdir -p ${OUTDIR}

# Samples for slurm array
ID=`sed -n ${SLURM_ARRAY_TASK_ID}p /scratch/djslab/jltung/Bfrag_project/HMP_metagenomes/HMP_metagenomes_mappingfile.txt`

# Run deconseq on forward paired reads, filtering out human genome
deconseq.pl \
-f ${INDIR}/${ID}.denovo_duplicates_marked.trimmed.1.fastq \
-out_dir ${OUTDIR} \
-id ${ID}.denovo_duplicates_marked.trimmed.1 \
-dbs hsref \
-t 8

# Run deconseq on reverse paired reads, filtering out human genome
deconseq.pl \
-f ${INDIR}/${ID}.denovo_duplicates_marked.trimmed.2.fastq \
-out_dir ${OUTDIR} \
-id ${ID}.denovo_duplicates_marked.trimmed.2 \
-dbs hsref \
-t 8
