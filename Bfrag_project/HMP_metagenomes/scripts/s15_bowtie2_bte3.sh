...
# Usage        : sbatch s15_bowtie2_bte3.sh
...
READSDIR=${BASEDIR}/HMP_metagenomes/bbmap_repaired
SAMPLE=`sed -n ${SLURM_ARRAY_TASK_ID}p ${BASEDIR}/HMP_metagenomes/HMP_metagenomes_mappingfile.txt`
REF=bte3
REF_INDEX=${BASEDIR}/bte_reference_sequences/${REF}/${REF} # Specify the full path to the index prefix

OUTDIR=${BASEDIR}/HMP_metagenomes/bowtie2_${REF}/${SAMPLE}_${REF}
