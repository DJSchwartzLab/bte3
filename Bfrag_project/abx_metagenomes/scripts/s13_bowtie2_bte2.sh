...
READSDIR=${BASEDIR}/abx_metagenomes/bbmap_repaired
SAMPLE=`sed -n ${SLURM_ARRAY_TASK_ID}p ${BASEDIR}/abx_metagenomes/abx_metagenomes_mappingfile.txt`
REF=bte2
REF_INDEX=${BASEDIR}/bte_reference_sequences/${REF}/${REF} # Specify the full path to the index prefix

OUTDIR=${BASEDIR}/abx_metagenomes/bowtie2_${REF}/${SAMPLE}_${REF}
